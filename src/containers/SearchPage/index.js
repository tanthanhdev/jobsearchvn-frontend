    ...
    <>
      ...
      <div className="pt-[100px] grid" style={{maxHeight: "58vh"}}>
        <div className="grid wide">
          ...
          <div className="roww">
            {jobs["results"] && jobs["results"].map((job, index) => {
                return (
                  <a className="coll cursor-pointer l-6 h-[260px] border rounded-2xl group" key={index} href={"/view-detail/" + job.id} target="_blank">
                    ...
                    <div className="flex">
                      ...
                      <span className="text-[#68BA50]">{job.salary ? job.salary + " " +job.currency : "Thương lượng"}</span>
                    </div>
                    <div className="flex justify-between">
                      <div>
                        {job.tag && job.tag.map((tagItem, index) => (
                          <span className="mr-2 border-border border-solid border text-[#6f6f6f] px-2" key={index}>
                            {tagItem.name}
                          </span>
                        ))}
                      </div>
                      <span className="text-[#6f6f6f]">{dateUtils.timeSince(new Date(job.created_at))}</span>
                    </div>
                  </a>
                );
              })}
